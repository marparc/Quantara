      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURES.map((f, i) => (
            <div
              key={f.title}
              className="relative p-6 rounded-2xl border border-white/6 hover:border-white/12 group cursor-default overflow-hidden transition-all duration-300"
              style={{
                background:
            ...
            >
              {/* Hover glow */}
              <div
                className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-linear-to-br ${f.gradient} pointer-events-none`}
              />

              <div
                className={`relative w-10 h-10 rounded-xl bg-linear-to-br ${f.gradient} border border-white/8 flex items-center justify-center ${f.accent} mb-5`}
              >
                {f.icon}
              </div>
